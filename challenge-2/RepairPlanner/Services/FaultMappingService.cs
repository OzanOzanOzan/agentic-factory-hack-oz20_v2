...
{
    // Static lookup: fault type → skill tags required to fix it.
    // StringComparer.OrdinalIgnoreCase makes lookups case-insensitive.
    ...
    // Static lookup: fault type → part numbers needed for repair.
    private static readonly IReadOnlyDictionary<string, IReadOnlyList<string>> FaultToParts =
        new Dictionary<string, IReadOnlyList<string>>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            ["high_radial_force_variation"]   = [],   // no parts needed; diagnosis only
    ...
    /// <summary>
    /// Returns the skill tags required to handle the given fault type.
    /// Falls back to ["general_maintenance"] for unknown fault types.
    /// </summary>
    public IReadOnlyList<string> GetRequiredSkills(string faultType) =>
        FaultToSkills.TryGetValue(faultType, out var skills)
            ? skills
            : ["general_maintenance"];  // ?? safe default for unknown faults

    /// <summary>
    /// Returns the part numbers needed to repair the given fault type.
    /// Falls back to an empty list for unknown fault types.
    /// </summary>
